...
/**
 * CustomDesignUtil is a utility in charge of checking the portal designs if customPortalDesigns config is true.
 * All PowerPortal designs should be stored as schematic files for this class to check.
 */
public class CustomDesignUtil {

    private static final FileConfiguration config = PowerPortals.getPlugin().getConfig();
    private static final ArrayList<Clipboard> designs = new ArrayList<>();
            ...
            for (File design : portalDesigns) {
                ...
                try {
                    ...
                    if (format != null) {
                        ...
                        if (checkDesign(clipboard, design)) {
                            designs.add(clipboard);
                        }

                    } else {
                        Bukkit.getLogger().warning("Unable to read " + design.getAbsolutePath() + " because of null format.");
                    }
     ...
     */
    private static boolean checkDesign(Clipboard clipboard, File file) {
     ...
     */
    public static PowerPortal attemptCreation(Player owner, Block lever) {
        ...
        for (Clipboard clipboard : designs) {
            portal = checkClipboard(clipboard, owner, lever, direction);
            if (portal != null) {
                return portal;
            }
        }
    ...
    private static PowerPortal checkClipboard (Clipboard clipboard, Player owner, Block lever, BlockFace direction) {
        ...
        if (isMatch) {
            ...
            return PortalCreationUtil.createPortal(owner, portals, triggers, clears, originIndex, leverIndex, signIndex);
        }
        return null;
    }
}
